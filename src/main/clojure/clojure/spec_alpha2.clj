...
(defn- explicate-1 [a-ns form]
  (walk/postwalk
    #(cond
       (keyword? %) %
       (symbol? %) (c/or (->> % (ns-resolve a-ns) ->sym) %)
       :else %)
    form))
...
(defmethod create-spec 'fn*
  [[_ & fn-tail]]
  (let [fn-form `(fn* ~@fn-tail)]
    (spec-impl (unfn fn-form) (eval fn-form) nil nil)))
...
(defn spec*
  [qform]
  (cond
    (keyword? qform) (get-spec qform)
    (symbol? qform) (spec-impl qform (deref (find-var qform)) nil nil)
...
(defmethod create-spec `spec
  [[_ form & {:keys [gen]}]]
  (spec-impl form (spec* form) (eval gen) nil))
...
(defmethod create-spec `merge
  [[_ & pred-forms]]
  (merge-spec-impl pred-forms (mapv spec* pred-forms) nil))

(declare every-impl)

(defmethod create-spec `every
  ...
  (let [gx (gensym)
        ...
        cpred `(fn* [~gx] (c/and ~@cpreds))
        eopts (-> opts
                  (dissoc ::gen)
                  (assoc ::cpred (eval cpred))
                  (cond->
                    kind (assoc :kind (eval kind) ::kind-form kind)))]
    (every-impl pred (spec* pred) eopts gen)))
...
(defmulti maybe-regex
  (fn [qform] (when (seqable? qform) (first qform))))

(defmethod maybe-regex :default [qform] nil)
...
(defmethod maybe-regex `*
  [[_ pred-form]]
  (rep-impl pred-form (c/or (maybe-regex pred-form) (spec* pred-form))))

(defmethod create-spec `*
  [[_ pred-form]]
  (specize (maybe-regex `(* ~pred-form))))
...
(defmethod maybe-regex `?
  [[_ pred-form]]
  (maybe-impl (c/or (maybe-regex pred-form) (spec* pred-form)) pred-form))
...
(defmethod maybe-regex `alt
  ...
  (let [pairs (partition 2 key-pred-forms)
    ...
    (alt-impl keys (mapv #(c/or (maybe-regex %) (spec* %)) pred-forms) pred-forms)))

(defmethod create-spec `alt
  [[_ & key-pred-forms]]
  (specize (maybe-regex `(alt ~@key-pred-forms))))
...
(defmethod maybe-regex `cat
  [[_ & key-pred-forms]]
  (let [pairs (partition 2 key-pred-forms)
        keys (mapv first pairs)
        pred-forms (mapv second pairs)]
    (c/assert (c/and (even? (count key-pred-forms)) (every? keyword? keys)) "cat expects k1 p1 k2 p2..., where ks are keywords")
    (cat-impl keys (mapv #(c/or (maybe-regex %) (spec* %)) pred-forms) pred-forms)))

(defmethod create-spec `cat
  [[_ & key-pred-forms]]
  (specize (maybe-regex `(cat ~@key-pred-forms))))
...
(defmacro &
  ...
  [re & preds]
  `(spec* (explicate '~(ns-name *ns*) '(& ~re ~@preds))))

(defmethod maybe-regex `&
  [[_ re & preds]]
  (amp-impl (c/or (maybe-regex re) (spec* re)) re (mapv eval preds) (mapv unfn preds)))

(defmethod create-spec `&
  [[_ re & preds]]
  (specize (maybe-regex `(& ~re ~@preds))))
...
(defmacro conformer
  ...
  ([f] `(spec* (explicate '(conformer ~f))))
  ([f unf] `(spec* (explicate '~(ns-name *ns*) '(conformer ~f ~unf)))))

(defmethod create-spec `conformer
  [[_ & [f unf]]]
  (if unf
    (spec-impl '(`conformer f unf) (eval f) nil true (eval unf))
    (spec-impl '(`conformer f) (eval f) nil true)))
...
(defmethod create-spec `fspec
  [[_ & {:keys [args ret fn gen] :or {ret `any?}}]]
  (fspec-impl (spec* args) args
              (spec* ret) ret
              (spec* fn) fn (eval gen)))
...
(defn- k-gen
  ...
  (cond
    ...
    (= `c/or (first f)) (or-k-gen 1 (rest f))
    (= `c/and (first f)) (and-k-gen (rest f))))
...
(defmethod create-spec `multi-spec
  [[_ mm retag]]
...
(defmethod create-spec `tuple
  [[_ & preds]]
  (tuple-impl (vec preds) (mapv spec* preds)))
...
(defmethod create-spec `or
  [[_ & key-pred-forms]]
  (let [pairs (partition 2 key-pred-forms)
        keys (mapv first pairs)
        pred-forms (mapv second pairs)
        pf (mapv spec* pred-forms)]
    (or-spec-impl keys pred-forms pf nil)))
...
(defmethod create-spec `and
  [[_ & pred-forms]]
  (and-spec-impl pred-forms (mapv spec* pred-forms) nil))
  ...
  (let [{:keys [::op ps ks forms splice p1 rep+ maybe amp] :as p} (reg-resolve! p)]
    ...
    (when p
      (case op
            ::accept nil
            nil p
            ::amp (list* 'clojure.spec-alpha2/& amp forms)
            ::pcat (if rep+
                     (list `+ rep+)
...
(defmacro keys*
  ...
  `(let [mspec# (keys ~@kspecs)]
     (with-gen (clojure.spec-alpha2/& (* (cat ::k keyword? ::v any?)) ::kvs->map mspec#)
       (fn [] (gen/fmap (fn [m#] (apply concat m#)) (gen mspec#))))))

(declare nonconforming)
...
(defmethod create-spec `nonconforming
  [[_ spec]]
  (nonconforming-impl (spec* spec)))
...
(defmethod create-spec `nilable
  [[_ pred]]
  (nilable-impl pred (spec* pred) nil))
