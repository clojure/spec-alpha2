        ...
        (testing "that the instrumented kwargs function operates as expected"
          ...
          (testing "that the ex-info data looks correct"
            (try (fail-no-kwargs 1 :not-num)
                 (catch Exception ei
                   (is (= 'clojure.test-clojure.instr/fail-no-kwargs (-> ei ex-data :clojure.alpha.spec.test/caller :var-scope)))))
        ...
        (testing "that the instrumented kwargs function operates as expected"
          ...
          (testing "that the ex-info data looks correct"
            (try (fail-kwargs 1 :not-num)
                 (catch Exception ei
                   (is (= 'clojure.test-clojure.instr/fail-kwargs (-> ei ex-data :clojure.alpha.spec.test/caller :var-scope)))))
